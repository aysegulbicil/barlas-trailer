...
- Üç bağımsız iç araç repoya gömülü, aynı Apache'den Alias ile sunulur (`deploy/apache-apps.conf` → compose mount; istekler ana CI4'e hiç girmez): **`/qr`** = `apps/qr` (ikinci bir CI4 4.7 uygulaması, personel/QR yoklama, MySQLi), **`/fatura`** = `apps/fatura` (saf PHP fatura takip, PDO), **`/teklif`** = `apps/teklif` (saf PHP+JS teklif üretici, DB'siz — JSON + `offers/` klasörleri).
- **Tek giriş + hub (`/patron`, 2026-07-06):** giriş `/patron/login.php`, girişten sonra üç kartlı animasyonlu hub `/patron/` — kimlik kaynağı **panelle aynı Shield kullanıcısı** (barlas.sqlite, düz PDO+password_verify). fatura+teklif `apps/_shared/auth/guard.php` (Apache `auto_prepend_file`) ile korunur; QR'a geçiş hub kartı → `/patron/go-qr.php` → 60 sn HMAC token (`writable/data/apps-sso.key`, otomatik üretilir) → `/qr/sso` (QR oturumunu e-posta eşleşmesi ya da ilk aktif admin ile açar). QR'ın kendi `/login`'i yedek kapıdır. Teklif'in statik dosyaları (offers/ müşteri verisi!) `apps/teklif/gate.php` üzerinden servis edilir. `/qr/q/{kod}` bilinçli girişsizdir (QR okutma).
