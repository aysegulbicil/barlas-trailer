...
/**
 * Shared layout for the embedded invoice tracking tool (/fatura).
 * Design (inline CSS + nav) ported verbatim from apps/fatura/partials/{header,footer}.php.
 * Expected data: $title (page title), $active (route name of the active nav item).
 */
$title  = $title ?? 'Fatura Takip';
$active = $active ?? '';

// route name => [label, svg path]
$nav = [
    'fatura'           => ['Genel Özet', 'M3 12l9-8 9 8M5 10v10h14V10'],
    'fatura.invoices'  => ['Faturalar', 'M7 3h10l3 3v15H4V6zM7 3v4h10'],
    'fatura.companies' => ['Firmalar', 'M3 21V8l7-4 7 4v13M9 21v-6h2v6'],
    'fatura.export'    => ['Excel\'e Aktar', 'M4 4h16v16H4zM4 9h16M9 9v11'],
    'fatura.import'    => ['İçe Aktar', 'M12 3v12m0 0l-4-4m4 4l4-4M4 21h16'],
];
?>
<!doctype html>
<html lang="tr">
...
<title><?= esc($title) ?> · Fatura Takip</title>
<style>
:root{
...
<header class="topbar">
  <div class="topbar-inner">
    <a href="<?= route_to('fatura') ?>" class="brand">
      <span class="logo">
    ...
    <nav class="nav">
      <?php foreach ($nav as $routeName => $item): ?>
        <a href="<?= route_to($routeName) ?>" class="<?= $active === $routeName ? 'active' : '' ?>">
...
<footer style="max-width:1480px;margin:0 auto;padding:24px;color:#94a3b8;font-size:12px;border-top:1px solid #e6e8f0;text-align:center">
  Fatura Takip · Barlas iç araçları · <?= date('Y') ?>
</footer>
</body>
</html>
