...
<div class="card">
    <?php if (empty($tasks)): ?>
        <?= view('qr/partials/empty', ['title' => 'Görev yok', 'message' => 'Sana atanmış bir görev bulunmuyor.']) ?>
    <?php else: ?>
            ...
            <div style="display:flex;gap:14px;align-items:flex-start;padding:14px 0;border-bottom:1px solid var(--line)">
                ...
                <div style="display:flex;gap:6px;flex-shrink:0">
                    <?php if (! in_array($t['status'], ['done', 'cancelled'], true)): ?>
                        ...
                        <form method="post" action="<?= qr_url('gorevlerim/' . $t['id'] . '/durum') ?>"><?= csrf_field() ?><input type="hidden" name="status" value="in_progress"><button class="btn btn-outline btn-sm">Başla</button></form>
                        <?php endif; ?>
                        <form method="post" action="<?= qr_url('gorevlerim/' . $t['id'] . '/durum') ?>"><?= csrf_field() ?><input type="hidden" name="status" value="done"><button class="btn btn-primary btn-sm">Tamamla</button></form>
