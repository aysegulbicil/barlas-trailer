...
$cur      = preg_replace('#^qr(/|$)#', '', $cur); // embedded under /qr: nav-active checks expect app-local paths
$role     = session()->get('role');
$name     = (string) session()->get('full_name');
$ini      = function_exists('initials') ? initials($name) : 'K';
$uid        = (int) session()->get('user_id');
$unread     = $loggedIn ? (new \App\Models\Qr\NotificationModel())->unreadCount($uid) : 0;
$pendingReq = ($loggedIn && $role === 'admin') ? (count((new \App\Models\Qr\LeaveRequestModel())->pending()) + count((new \App\Models\Qr\AdvanceRequestModel())->pending())) : 0;
$openTasks  = 0;
if ($loggedIn && $role !== 'admin') {
    try { $openTasks = (new \App\Models\Qr\TaskModel())->countOpenForUser($uid); } catch (\Throwable $e) { $openTasks = 0; }
}
$asset      = static fn (string $p): string => base_url('qr/' . $p) . '?v=' . (@filemtime(FCPATH . 'qr/' . $p) ?: 1);
?>
<!doctype html>
<html lang="tr">
<head>
...
<div class="app-shell">
    <aside class="sidebar">
        <a class="brand" href="<?= qr_url('dashboard') ?>">
        ...
        <nav class="nav-group">
            ...
            <a class="nav-link <?= $cur === 'dashboard' ? 'active' : '' ?>" href="<?= qr_url('dashboard') ?>">
            ...
            <a class="nav-link <?= $cur === 'history' ? 'active' : '' ?>" href="<?= qr_url('history') ?>">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>
                Geçmişim
            </a>
            <a class="nav-link <?= $cur === 'requests' ? 'active' : '' ?>" href="<?= qr_url('requests') ?>">
            ...
            <a class="nav-link <?= $cur === 'gorevlerim' ? 'active' : '' ?>" href="<?= qr_url('gorevlerim') ?>">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>
                Görevlerim
                <?php if ($openTasks): ?><span class="nav-badge"><?= $openTasks ?></span><?php endif; ?>
            </a>
            <a class="nav-link <?= $cur === 'notifications' ? 'active' : '' ?>" href="<?= qr_url('notifications') ?>">
        ...
        <nav class="nav-group">
            ...
            <a class="nav-link <?= $cur === 'admin' ? 'active' : '' ?>" href="<?= qr_url('admin') ?>">
            ...
            <a class="nav-link <?= str_starts_with($cur, 'admin/requests') ? 'active' : '' ?>" href="<?= qr_url('admin/requests') ?>">
            ...
            <a class="nav-link <?= str_starts_with($cur, 'admin/tasks') ? 'active' : '' ?>" href="<?= qr_url('admin/tasks') ?>">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>
                Görevler
            </a>
            <a class="nav-link <?= str_starts_with($cur, 'admin/employees') ? 'active' : '' ?>" href="<?= qr_url('admin/employees') ?>">
            ...
            <a class="nav-link <?= str_starts_with($cur, 'admin/departments') ? 'active' : '' ?>" href="<?= qr_url('admin/departments') ?>">
            ...
            <a class="nav-link <?= str_starts_with($cur, 'admin/positions') ? 'active' : '' ?>" href="<?= qr_url('admin/positions') ?>">
            ...
            <a class="nav-link <?= (str_starts_with($cur, 'admin/shifts') || str_starts_with($cur, 'admin/shift-schedule')) ? 'active' : '' ?>" href="<?= qr_url('admin/shifts') ?>">
            ...
            <a class="nav-link <?= str_starts_with($cur, 'admin/locations') ? 'active' : '' ?>" href="<?= qr_url('admin/locations') ?>">
            ...
            <a class="nav-link <?= str_starts_with($cur, 'admin/suspicious') ? 'active' : '' ?>" href="<?= qr_url('admin/suspicious') ?>">
            ...
            <a class="nav-link <?= str_starts_with($cur, 'admin/attendance') ? 'active' : '' ?>" href="<?= qr_url('admin/attendance') ?>">
            ...
            <a class="nav-link <?= str_starts_with($cur, 'admin/payroll') ? 'active' : '' ?>" href="<?= qr_url('admin/payroll') ?>">
            ...
            <a class="nav-link <?= str_starts_with($cur, 'admin/reports') ? 'active' : '' ?>" href="<?= qr_url('admin/reports') ?>">
            ...
            <a class="nav-link <?= str_starts_with($cur, 'admin/settings') ? 'active' : '' ?>" href="<?= qr_url('admin/settings') ?>">
        ...
        <a class="nav-link" href="<?= qr_url('logout') ?>">
