...
<div class="card-head">
    ...
    <a class="btn btn-outline" href="<?= qr_url('admin/shifts') ?>">Vardiyalar</a>
</div>

<form method="get" action="<?= qr_url('admin/shift-schedule') ?>" class="filters">
    ...
    <a class="btn btn-outline btn-sm" href="<?= qr_url('admin/shift-schedule') . '?user_id=' . $userId . '&week=' . $prevWeek ?>">‹ Önceki</a>
    <span class="muted"><?= esc(date('d.m.Y', strtotime($weekStart))) ?> haftası</span>
    <a class="btn btn-outline btn-sm" href="<?= qr_url('admin/shift-schedule') . '?user_id=' . $userId . '&week=' . $nextWeek ?>">Sonraki ›</a>
</form>

<?php if (! $userId): ?>
    <div class="card"><?= view('qr/partials/empty', ['title' => 'Personel seç', 'message' => 'Plan görüntülemek için yukarıdan bir personel seç.']) ?></div>
<?php elseif (empty($shifts)): ?>
    <div class="card"><?= view('qr/partials/empty', ['title' => 'Önce vardiya tanımla', 'message' => 'Plan yapabilmek için en az bir vardiya gerekli.', 'actionUrl' => qr_url('admin/shifts/new'), 'actionLabel' => '+ Yeni vardiya', 'actionModal' => true, 'actionTitle' => 'Yeni vardiya']) ?></div>
<?php else: ?>
<form method="post" action="<?= qr_url('admin/shift-schedule') ?>">
    <?= csrf_field() ?>
