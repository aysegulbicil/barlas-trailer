...
<div class="card-head">
    ...
    <div class="btn-group">
        <a class="btn btn-primary" href="<?= qr_url('admin/attendance/new') ?>" data-modal data-modal-title="Elle devam kaydı">+ Elle kayıt ekle</a>
    </div>
</div>
            ...
            <?php foreach ($openLogs as $o): ?>
                <tr>
                    ...
                    <td class="row-actions">
                        <a class="btn btn-primary btn-sm" href="<?= qr_url('admin/attendance/new') . '?user_id=' . (int) $o['user_id'] . '&type=out' ?>" data-modal data-modal-title="Çıkış ekle">Çıkış ekle</a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
...
<div class="card">
    <?php if (empty($logs)): ?>
        <?= view('qr/partials/empty', ['title' => 'Kayıt yok', 'message' => 'Henüz giriş/çıkış kaydı yok.']) ?>
    <?php else: ?>
    <div class="table-scroll">
        <table class="data">
            ...
            <?php foreach ($logs as $l): ?>
                <tr>
                    ...
                    <td class="row-actions">
                        <a class="btn btn-warning-soft btn-sm" href="<?= qr_url('admin/attendance/' . $l['id'] . '/edit') ?>" data-modal data-modal-title="Kaydı düzenle">Düzenle</a>
                        <form method="post" action="<?= qr_url('admin/attendance/' . $l['id'] . '/delete') ?>" data-confirm="Bu kayıt silinsin mi? Puantajı etkileyebilir."><?= csrf_field() ?><button class="btn btn-danger-soft btn-sm">Sil</button></form>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
