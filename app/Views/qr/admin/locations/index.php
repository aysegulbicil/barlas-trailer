...
<div class="card-head">
    ...
    <?php else: ?>
        <a class="btn btn-primary" href="<?= qr_url('admin/locations/new') ?>" data-modal data-modal-title="Yeni lokasyon">+ Yeni lokasyon</a>
    <?php endif; ?>
</div>
<?php if ($atLimit): ?>
...
<div class="card">
    <?php if (empty($locations)): ?>
        <?= view('qr/partials/empty', ['title' => 'Henüz lokasyon yok', 'message' => 'İlk lokasyonunu ekle.', 'actionUrl' => qr_url('admin/locations/new'), 'actionLabel' => '+ Yeni lokasyon', 'actionModal' => true, 'actionTitle' => 'Yeni lokasyon']) ?>
    <?php else: ?>
            ...
            <?php foreach ($locations as $l): ?>
                <tr>
                    ...
                    <td class="row-actions">
                        <a href="<?= qr_url('admin/locations/' . $l['id'] . '/qr') ?>">QR</a>
                        <a class="btn btn-warning-soft btn-sm" href="<?= qr_url('admin/locations/' . $l['id'] . '/edit') ?>" data-modal data-modal-title="Lokasyonu düzenle">Düzenle</a>
                        <form method="post" action="<?= qr_url('admin/locations/' . $l['id'] . '/delete') ?>" data-confirm="Bu lokasyon silinsin mi?"><?= csrf_field() ?><button class="btn btn-danger-soft btn-sm">Sil</button></form>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
