...
<?php $cur = $p['currency']; ?>
<p class="back-link"><a href="<?= qr_url('admin/payroll') . '?month=' . esc($ym) ?>">&larr; Puantaj</a></p>
...
<div class="card pad-lg">
    <div class="profile-head">
        ...
        <div class="pactions">
            <form method="get" action="<?= qr_url('admin/payroll/' . $u['id']) ?>" style="margin:0">
        ...
        <?php else: ?>
            <?php foreach ($advances as $a): ?>
                <div class="info-row">
                    ...
                    <?php if (empty($closed)): ?><form method="post" action="<?= qr_url('admin/payroll/' . $u['id'] . '/advance/' . $a['id'] . '/delete') ?>" data-confirm="Silinsin mi?"><?= csrf_field() ?><input type="hidden" name="month" value="<?= esc($ym) ?>"><button class="btn btn-link btn-sm" style="color:var(--dng-ink)">Sil</button></form><?php endif; ?>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>

        <?php if (empty($closed)): ?>
        <form method="post" action="<?= qr_url('admin/payroll/' . $u['id'] . '/advance') ?>" style="margin-top:16px;border-top:1px solid var(--line);padding-top:16px">
            <?= csrf_field() ?>
