...
<div class="card-head">
    <h1>Puantaj &amp; Maaş</h1>
    <form method="get" action="<?= qr_url('admin/payroll') ?>" class="m0">
...
<div class="card-head" style="margin-top:-4px">
    ...
    <div class="btn-group">
        <?php if (! empty($closed)): ?>
            ...
            <form method="post" action="<?= qr_url('admin/payroll/reopen') ?>" class="m0" data-confirm="Bu ay yeniden açılsın mı? Değerler tekrar canlı hesaplanır." data-confirm-safe data-confirm-title="Ayı yeniden aç" data-confirm-ok="Yeniden aç"><?= csrf_field() ?><input type="hidden" name="month" value="<?= esc($ym) ?>"><button class="btn btn-outline btn-sm">Yeniden aç</button></form>
        <?php else: ?>
            <form method="post" action="<?= qr_url('admin/payroll/close') ?>" class="m0" data-confirm="<?= esc($monthStr) ?> dondurulsun mu? Bu aydaki değerler sabitlenir, sonraki düzeltmelerden etkilenmez." data-confirm-safe data-confirm-title="Ayı dondur" data-confirm-ok="Ayı dondur"><?= csrf_field() ?><input type="hidden" name="month" value="<?= esc($ym) ?>"><button class="btn btn-primary btn-sm">Ayı dondur</button></form>
        <?php endif; ?>
    </div>
    <?php endif; ?>
</div>
...
<div class="card">
    <?php if (empty($rows)): ?>
        <?= view('qr/partials/empty', ['title' => 'Personel yok', 'message' => 'Puantaj için önce personel ekle.', 'actionUrl' => qr_url('admin/employees/new'), 'actionLabel' => '+ Yeni personel', 'actionModal' => true, 'actionTitle' => 'Yeni personel', 'actionSize' => 'lg']) ?>
    <?php else: ?>
    <div class="table-scroll">
        <table class="data">
            ...
            <?php foreach ($rows as $row): $u = $row['user']; $p = $row['p']; ?>
                <tr>
                    <td>
                        <a class="cell-user" href="<?= qr_url('admin/payroll/' . $u['id']) . '?month=' . esc($ym) ?>">
                    ...
                    <td class="text-right"><a class="btn btn-outline btn-sm" href="<?= qr_url('admin/payroll/' . $u['id']) . '?month=' . esc($ym) ?>">Detay</a></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
