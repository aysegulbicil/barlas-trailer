...
<div class="card-head">
    ...
    <div class="btn-group">
        <a class="btn btn-primary" href="<?= qr_url('admin/tasks/new') ?>" data-modal data-modal-title="Görev ata" data-modal-size="lg">+ Görev ata</a>
    </div>
</div>

<form method="get" action="<?= qr_url('admin/tasks') ?>" class="filters">
    ...
    <?php if ($fUser || $fStatus !== ''): ?><a class="btn btn-link btn-sm" href="<?= qr_url('admin/tasks') ?>">Filtreyi temizle</a><?php endif; ?>
</form>

<div class="card">
    <?php if (empty($tasks)): ?>
        <?= view('qr/partials/empty', ['title' => 'Görev yok', 'message' => 'İlk görevini atayarak başla.', 'actionUrl' => qr_url('admin/tasks/new'), 'actionLabel' => '+ Görev ata', 'actionModal' => true, 'actionTitle' => 'Görev ata', 'actionSize' => 'lg']) ?>
    <?php else: ?>
            ...
            <?php foreach ($tasks as $t): ?>
                ...
                <tr>
                    ...
                    <td class="row-actions">
                        <a class="btn btn-warning-soft btn-sm" href="<?= qr_url('admin/tasks/' . $t['id'] . '/edit') ?>" data-modal data-modal-title="Görevi düzenle" data-modal-size="lg">Düzenle</a>
                        <form method="post" action="<?= qr_url('admin/tasks/' . $t['id'] . '/delete') ?>" data-confirm="Bu görev silinsin mi?"><?= csrf_field() ?><button class="btn btn-danger-soft btn-sm">Sil</button></form>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
