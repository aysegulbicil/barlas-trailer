...
use App\Services\Qr\EmployeeService;
...
class Employees extends BaseController
{
    public function index()
    {
        ...
        return view('qr/admin/employees/index', [
    ...
    public function bulk()
    {
        ...
        if ($ids === []) {
            return redirect()->to(qr_url('admin/employees'))->with('error', 'Hiç personel seçilmedi.');
        }
        ...
        if (isset($labels[$action])) {
            ...
            return redirect()->to(qr_url('admin/employees'))->with('message', "{$n} personel durumu güncellendi: {$labels[$action]}.");
        }

        return redirect()->to(qr_url('admin/employees'))->with('error', 'Geçersiz toplu işlem.');
    }
    ...
    public function edit(int $id)
    {
        ...
        if ($employee === null) {
            return redirect()->to(qr_url('admin/employees'))->with('error', 'Personel bulunamadı.');
        }
    ...
    private function form(?array $employee)
    {
        ...
        return view($this->wantsJson() ? 'qr/admin/employees/_form' : 'qr/admin/employees/form', $data);
    }

    public function create()
    {
        // validateData + 'qr' dbGroup: the is_unique rule must query the QR
        // database, not the main site's default (SQLite/Shield) connection.
        if (! $this->validateData($this->request->getPost(), $this->rules(), [], 'qr')) {
        ...
        return $this->wantsJson()
            ? $this->jsonOk(qr_url('admin/employees'), 'Personel eklendi.')
            : redirect()->to(qr_url('admin/employees'))->with('message', 'Personel eklendi.');
    }

    public function update(int $id)
    {
        // validateData + 'qr' dbGroup: is_unique must hit the QR database (see create()).
        if (! $this->validateData($this->request->getPost(), $this->rules($id), [], 'qr')) {
            $msg = implode(' ', $this->validator->getErrors());
        ...
        return $this->wantsJson()
            ? $this->jsonOk(qr_url('admin/employees'), 'Personel güncellendi.')
            : redirect()->to(qr_url('admin/employees'))->with('message', 'Personel güncellendi.');
    }
    ...
    public function show(int $id)
    {
        ...
        return view('qr/admin/employees/profile', [
            'emp'     => $emp,
            'tLate'   => $tLate,
            'tOt'     => $tOt,
            'tWorked' => $tWorked,
