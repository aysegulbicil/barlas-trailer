...
use App\Models\Qr\PayrollRunModel;
use App\Models\Qr\UserModel;
use App\Services\Qr\PayrollService;
...
class Payroll extends BaseController
{
    public function index()
    {
        ...
        return view('qr/admin/payroll/index', [
            'rows'     => $rows,
            'year'     => $year,
            'month'    => $month,
    ...
    public function show(int $id)
    {
        ...
        if ($u === null) {
            return redirect()->to(qr_url('admin/payroll'))->with('error', 'Personel bulunamadı.');
        }
        ...
        return view('qr/admin/payroll/detail', [
            'u'        => $u,
            'p'        => $p,
    ...
    {
        return qr_url('admin/payroll') . '?month=' . sprintf('%04d-%02d', $year, $month);
    }

    private function detailUrl(int $id, int $year, int $month): string
    {
        return qr_url('admin/payroll/' . $id) . '?month=' . sprintf('%04d-%02d', $year, $month);
    }
}
