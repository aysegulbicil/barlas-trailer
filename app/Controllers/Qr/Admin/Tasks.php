...
class Tasks extends BaseController
{
    public function index()
    {
        ...
        return view('qr/admin/tasks/index', [
    ...
    public function new()
    {
        return view($this->wantsJson() ? 'qr/admin/tasks/_form' : 'qr/admin/tasks/form', $this->formData(null));
    }
    ...
    public function edit(int $id)
    {
        ...
        if ($task === null) {
            return redirect()->to(qr_url('admin/tasks'))->with('error', 'Görev bulunamadı.');
        }

        return view($this->wantsJson() ? 'qr/admin/tasks/_form' : 'qr/admin/tasks/form', $this->formData($task));
    }
    ...
    public function create()
    {
        ...
        foreach ($this->selectedUsers() as $uid) {
            ...
            $notifier->notify($uid, 'task', 'Sana yeni görev atandı: ' . $base['title'], qr_url('gorevlerim'));
            $count++;
        }
        ...
        return $this->wantsJson()
            ? $this->jsonOk(qr_url('admin/tasks'), $msg)
            : redirect()->to(qr_url('admin/tasks'))->with('message', $msg);
    }
    ...
    public function update(int $id)
    {
        ...
        if ($task === null) {
            ...
            return $this->wantsJson() ? $this->jsonError($msg) : redirect()->to(qr_url('admin/tasks'))->with('error', $msg);
        }
        if ($error = $this->validatePayload()) {
            return $this->wantsJson() ? $this->jsonError($error) : redirect()->back()->withInput()->with('error', $error);
        }
        ...
        return $this->wantsJson()
            ? $this->jsonOk(qr_url('admin/tasks'), 'Görev güncellendi.')
            : redirect()->to(qr_url('admin/tasks'))->with('message', 'Görev güncellendi.');
    }
    ...
    public function delete(int $id)
    {
        ...
        return redirect()->to(qr_url('admin/tasks'))->with('message', 'Görev silindi.');
    }
