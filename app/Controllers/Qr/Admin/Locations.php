...
use App\Services\Qr\DynamicQr;

class Locations extends BaseController
{
    public function index()
    {
        return view('qr/admin/locations/index', [
    ...
    public function new()
    {
        if ($error = $this->locationLimitError()) {
            return redirect()->to(qr_url('admin/locations'))->with('error', $error);
        }

        return view($this->wantsJson() ? 'qr/admin/locations/_form' : 'qr/admin/locations/form', ['location' => null, 'regen' => $this->regenInfo()]);
    }
    ...
    public function edit(int $id)
    {
        ...
        if ($location === null) {
            return redirect()->to(qr_url('admin/locations'))->with('error', 'Lokasyon bulunamadı.');
        }

        return view($this->wantsJson() ? 'qr/admin/locations/_form' : 'qr/admin/locations/form', ['location' => $location, 'regen' => $this->regenInfo()]);
    }

    public function create()
    {
        if ($error = $this->locationLimitError()) {
            return $this->wantsJson() ? $this->jsonError($error) : redirect()->to(qr_url('admin/locations'))->with('error', $error);
        }
        ...
        return $this->wantsJson() ? $this->jsonOk(qr_url('admin/locations'), $msg) : redirect()->to(qr_url('admin/locations'))->with('message', $msg);
    }
    ...
    public function update(int $id)
    {
        ...
        $settings = new \App\Models\Qr\SettingModel();
    ...
    public function delete(int $id)
    {
        ...
        if ($logCount > 0) {
            return redirect()->to(qr_url('admin/locations'))
        ...
        return redirect()->to(qr_url('admin/locations'))->with('message', 'Lokasyon silindi.');
    }
    ...
    public function qr(int $id)
    {
        ...
        return view('qr/admin/locations/qr', ['location' => $location]);
    }
    ...
    public function token(int $id)
    {
        ...
        $base = rtrim(base_url('qr'), '/') . '/q/' . $location['code'];
    ...
    private function regenInfo(): array
    {
        return [
            'limit' => fixed_qr_regen_limit(),
            'used'  => (int) (new \App\Models\Qr\SettingModel())->getValue('fixed_qr_regen_used', '0'),
        ];
    }
