 ...
 * URL: /qr/display/{id}/{token_secret}. Admin oturumu TASIMAZ; kapi cihazi
...
class Display extends BaseController
{
    public function screen(int $id, string $secret)
    {
        ...
        return view('qr/display/screen', [
            'location' => $location,
            'mode'     => qr_effective_mode($location['qr_mode']),
            'tokenUrl' => qr_url('display/' . $location['id'] . '/' . $location['token_secret'] . '/token'),
        ]);
    }
